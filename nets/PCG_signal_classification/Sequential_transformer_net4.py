...
class TransformerNet(nn.Module):
    ...
    def __init__(self, net_params):
        ...
        self.convlayer2 = nn.Sequential(
            nn.Conv2d(in_channels=3, out_channels=6, kernel_size=1, padding=1),
            nn.BatchNorm2d(6),
            # nn.ReLU()
            nn.MaxPool2d(2)
        )
        ...
        if self.kerSize == 3:
            d_model = 6 * 12  # node_dim (feat is an integer)
        else:
            d_model = 6 * 12*2  # node_dim (feat is an integer)
    ...
    def forward(self, h):
        ...
        if self.HWorder == True:
            ...
            h = h.reshape([L * W, 1, 44, 2])
        else:
            # h = h.reshape([L * W, 1, 3, 64])# for singlen=192
            h = h.reshape([L * W, 1, 2, 44])
