...
##https://pythonguides.com/pytorch-conv1d/
        ...
        if layer_norm == False and batch_norm == False:
            self.convlayer1 = nn.Sequential(
                nn.Conv1d(in_channels=64, out_channels=128, kernel_size=self.kerSize, padding=1),
                nn.Conv1d(in_channels=128, out_channels=256, kernel_size=self.kerSize, padding=1),
                nn.Conv1d(in_channels=256, out_channels=128, kernel_size=self.kerSize, padding=1),
                nn.Conv1d(in_channels=128, out_channels=64, kernel_size=1, padding=1),
                self.act,
        ...
        elif batch_norm == True and layer_norm == False:
            self.convlayer1 = nn.Sequential(
                nn.Conv1d(in_channels=64, out_channels=128, kernel_size=self.kerSize, padding=1),
                nn.BatchNorm1d(128),
                nn.Conv1d(in_channels=128, out_channels=256, kernel_size=self.kerSize, padding=1),
            ...
            self.convlayer2 = nn.Sequential(
                nn.Conv1d(in_channels=64, out_channels=128, kernel_size=1, padding=1),
                nn.BatchNorm2d(128),
                self.act,
                nn.MaxPool2d(2)
            )
        ...
        if self.kerSize == 3:
            d_model = 64  # node_dim (feat is an integer)
        elif layer_norm == True and batch_norm == True:
            d_model = 128
        else:
            d_model = 160  # node_dim (feat is an integer)
